...
def find_max_interval2(simulation_time, dic_type_num):
    ...
    total_fleets = sum(dic_type_num.values())
    # Total veh number
    result_hv_num = sum(int(k.split()[-1]) * v for k, v in dic_type_num.items())
    total_veh_num = total_fleets + result_hv_num
    ...
    # Calculate the maximum possible interval
    max_interval = (simulation_time - total_veh_num)//(total_fleets - 1)
    return max_interval
...
def get_schedule(st, p, fr, max_attempts=5, plot=False, seed=None, display=False):
    ...
    max_interval2 = find_max_interval2(st, dic_tn)
    dp_times_dict, dp_times = find_optimse_schedule(dic_tn, st, max_interval2, \
