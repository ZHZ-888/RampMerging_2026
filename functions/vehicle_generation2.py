...
def vdp(p):
    ...
    r = {k: v for k, v in distribution.items() if round(v, 2) > 0.05}  # round(v,2) 保留两位小数
    return r
...
def generate_type_num2(percentage, flow_rate, simulation_time, seed=None):
    ...
    for num_type1_0 in range(0, group_num + 1):
        for _ in range(100):  # Limit the number of attempts for random assignments
...
# drop tqdm
def find_optimse_schedule2(dic_tn, st, max_interval, max_attempts=50, seed=None):
    while (max_interval >= 0):
...
def plot_platoon_groups(dp_times_dict, color_map=None):
    """
    Plot the platoon groups and their departure times.
    the length of box is the veh number of fleet
    # 小方框的长度就是车队中车的数量

    Parameters:
        dp_times_dict (dict): Dictionary with fleet types as keys and departure \
            times as values.
        dp_times (list): List of departure times.
        color_map (dict): Optional. Dictionary with fleet types as keys and colors \
            as values. Default colors will be used if not provided.
    """
    ...
    data_items = []
    for fleet_type, times in dp_times_dict.items():
        color = color_map.get(fleet_type, 'black')
        num_vehicles = int(fleet_type.split()[-1]) + 1
        label = f'Type {num_vehicles} ({fleet_type})'
        data_items.extend((time, color, num_vehicles, label) for time in times)

    # Sort data items by departure time
    sorted_data_items = sorted(data_items, key=lambda x: x[0])

    # Create figure and axis
    fig, ax = plt.subplots(figsize=(5, 20), dpi=150)

    # Track whether each label has been added
    labels_added = {label: False for _, _, _, label in sorted_data_items}

    # Plot the bars
    for idx, (value, color, width, label) in enumerate(sorted_data_items):
        if not labels_added[label]:
            ax.barh(idx, width, left=value, height=0.3, color=color, label=label)
            labels_added[label] = True
        else:
            ax.barh(idx, width, left=value, height=0.3, color=color)

    # Set y-axis and x-axis labels and title
    ax.set_yticks(range(len(sorted_data_items)))
    ax.set_yticklabels(range(1, len(sorted_data_items) + 1))
    ax.set_ylabel('Groups')
    ax.set_xlabel('Departure time (s)')
    ax.set_title('Platoon groups and departure time')
    ax.legend()
    ax.grid()

    # Set x-axis range
    max_value = max(data[0] + data[2] for data in sorted_data_items)
    ax.set_xlim(0, max_value + 5)  # Add 5 as a buffer
...
class VehGen:
    ...
    def add_vehicle(self, vehicle_id, route_id, dp_v, type_id="idm"):
        ...
        # self.traci.vehicle.setSpeedMode(vehicle_id, 0b010001)
        # self.traci.vehicle.setSpeedMode(vehicle_id, 0b010111)
        self.traci.vehicle.setSpeedMode(vehicle_id, 0b010111)

    def veh_gen_hv(self, step, ls_departure_time, id_prefix, veh_route, dp_speed):
        for dt in ls_departure_time:
            r_step = step/10
            if r_step == dt:
                id = id_prefix + str(int(r_step))
                self.add_vehicle(id, veh_route, dp_speed)

    def veh_gen_hv2(self, step, ls_departure_time, id_prefix, veh_route, dp_speed):
        '''
        introduce HV hetergenity
        :param step:
        :param ls_departure_time:
        :param id_prefix:
        :param veh_route:
        :param dp_speed:
        :return:
    ...
    # id_prefix = 'r' or 'm'
    def veh_gen_mix(self, step, dp_times_dict, id_prefix, \
                 veh_route, dp_v):
        '''
...
if __name__ == '__main__':
    ...
    max_attempts = 1
    seed = None
    dic_dpt_type = get_schedule(st, p, fr, max_attempts, seed)
