...
def get_schedule(st, p, fr, max_attempts=5, plot=False, seed=None, display=False):
    """
    combine above function
    generate departure time of every platoons
    :param st: simulation time (seconds)
    :param p:
    :param fr:
    :param seed:
    ...
    if plot:
        # plot_departure_times(dic_time_type, p, fr, st, seed) # before reorganise
        plot_departure_times(result, p, fr, st, seed, display) # after reorganise
...
class VehGen:
    ...
    def add_vehicle(self, vehicle_id, route_id, dp_v, dp_lane, type_id="idm"):
        ...
        for dt, type in dp_times_dict.items():
            ...
            for i, veh in enumerate(type):
                ...
                if dt_v == r_step:
                    ...
                    if dp_lane == '1': # lane 1 => 'b'
                        id_suffix = id_prefix + dp_lane + id_body
                    else:
                        id_suffix = id_prefix + id_body
                    id = f"{id_suffix}{step}"
                    if id == 'rhv4030':
                        pass
                    self.add_vehicle(id, veh_route, dp_v, dp_lane, vehicle_type)

    def veh_gen_mw(self, step, dp_times_dict, id_prefix, \
                 veh_route, dp_v, dp_lane='0'):
        '''
        mw: motorway
        ...
        if c_ts in dp_times_dict:
            ...
            if id == 'mhv18':
                pass
            self.add_vehicle(id, veh_route, dp_v, dp_lane, mode)
